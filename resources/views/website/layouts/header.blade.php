<nav>
    ...
    <ul>
        <li style="float: right">
            <a href="/"><img style="width: 62px;margin-top: -8px" alt="{{conf("system_title")}}"
                             src="{{asset("site/images/logo-inverse.png")}}"/></a>
        </li>
        <li style="float: right">
            <a href="/"> خانه </a>
        </li>
    ...
    <div class="others-options d-flex align-items-center header-m">
        @auth
            <sub-component fullname="{{auth()->user()->name.' '.auth()->user()->last_name}}"
                           img="{{user_picture(auth()->user()->id , 'user-icon' , true)}}"></sub-component>
        @endauth
